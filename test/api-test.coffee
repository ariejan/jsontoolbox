  ...
  .addBatch
    'start test server':
      topic: ->
        ...
        app.listen 50666, this.callback
      'should have started': -> null

  .addBatch
    #### Ping
    'Ping':
      topic: -> zombie.visit('http://localhost:50666/api/ping', this.callback)
    ...
    #### IP Address
    'IP Address':
      topic: -> zombie.visit('http://localhost:50666/api/ip', this.callback)
    ...
    #### User Agent
    'User agent':
      topic: -> zombie.visit('http://localhost:50666/api/user-agent', this.callback)
    ...
    "Random password":
      'with default length':
        topic: -> zombie.visit('http://localhost:50666/api/password', this.callback)
      ...
      'with length 40':
        topic: -> zombie.visit('http://localhost:50666/api/password?length=40', this.callback)

        'should receive a password of length 40': (browser) ->
          JSON.parse(browser.text('body'))['password'].length.should.equal(40)

      'with a length > 256':
        topic: -> zombie.visit('http://localhost:50666/api/password?length=5028', this.callback)
      ...
      'with a negative length of -32':
        topic: -> zombie.visit('http://localhost:50666/api/password?length=-32', this.callback)
